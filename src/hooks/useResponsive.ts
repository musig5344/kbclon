...
import { calculateResponsiveSize, getDPIScale, KB_DESIGN_TOKENS } from '../styles/responsive-system';
...
export const useResponsive = () => {
  ...
  // 반응형 폰트 크기 계산
  const getResponsiveFontSize = useCallback((baseFontSize: number): string => {
  ...
  // 반응형 간격 계산
  const getResponsiveSpacing = useCallback((baseSpacing: keyof typeof KB_DESIGN_TOKENS.spacing): string => {
  ...
  // 반응형 크기 계산
  const getResponsiveSize = useCallback((baseSize: number, minSize?: number, maxSize?: number): number => {
  ...
  const isKBCompatible = useMemo(() => {
    return responsiveState.width >= KB_BREAKPOINTS.kbMinimum && 
