 ...
 * 조건이 true일 때만 값을 재계산
 */
export function useConditionalMemo<T>(factory: () => T, condition: boolean, fallback: T): T {
 ...
 * 성능 모니터링이 포함된 메모이제이션
 */
export function usePerformanceMemo<T>(factory: () => T, deps: DependencyList, label?: string): T {
  return useMemo(() => {
...
export function useEventHandler<T extends (...args: any[]) => any>(handler: T): T {
  ...
  // 안정된 참조 반환
  const stableHandler = useCallback(((...args: any[]) => handlerRef.current(...args)) as T, []);
...
export function useHookPerformance(hookName: string) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (renderCount.current % 100 === 0) {
      console.log(
        `${hookName} - 렌더링 횟수: ${renderCount.current}, 평균 시간: ${renderTime.toFixed(2)}ms`
      );
    }
  }
