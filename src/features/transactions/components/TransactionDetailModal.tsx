        ...
        <DetailInfoSection>
          ...
          <DetailInfoRow>
            ...
            <DetailInfoValue $isAmount={true} $isPositive={transaction.transaction_type === '입금'}>
              {transaction.transaction_type === '이체' ? '출금' : transaction.transaction_type} {formatCurrency(Math.abs(transaction.amount))}원
            </DetailInfoValue>
          </DetailInfoRow>
          <DetailInfoRow>
          ...
          <DetailInfoRow>
            ...
            <DetailInfoValue>
              {transaction.transaction_type === '이체' ? 'CMS 공동' : 
               transaction.transaction_type === '입금' ? '타행이체' : 
               transaction.transaction_type === '출금' ? '전자금융' : '기타'}
            </DetailInfoValue>
          </DetailInfoRow>
        </DetailInfoSection>
      </DetailBody>
      <DetailConfirmButton onClick={onClose}>
        확인
      </DetailConfirmButton>
    </ModalContainer>
  );
