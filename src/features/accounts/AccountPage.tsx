...
const AccountPage: React.FC = () => {
  ...
  // 샘플 거래내역 생성 함수
  const getSampleTransactions = useCallback(
    (): Transaction[] => [
      {
        id: '1',
        date: '2025.05.20',
      ...
      try {
        ...
        if (transactionError) {
          ...
          const convertedTransactions: Transaction[] =
            transactionData?.map((tx, index: number) => ({
              id: tx.id ? String(tx.id) : String(index),
              date: tx.transaction_date
                ? new Date(tx.transaction_date).toISOString().split('T')[0].replace(/-/g, '.')
                : '2025.07.21',
              time: tx.transaction_date
                ? new Date(tx.transaction_date).toTimeString().split(' ')[0]
                : '12:00:00',
          ...
          setTransactions(
            convertedTransactions.length > 0 ? convertedTransactions : getSampleTransactions()
          );
        }
      } catch (error) {
        // 에러 때 Mock 데이터 사용
  ...
  return (
    <AccountPageContainer>
      ...
      <MainContent>
        ...
        <FilterSection>
          ...
          <FilterOptions>
            ...
            <ToggleSwitch enabled={showBalance} onClick={handleToggleBalance} />
          </FilterOptions>
        </FilterSection>
        {/* 날짜 범위 */}
        <DateRangeSection>
      ...
      <MoreOptionsModal show={showMoreOptions} onClose={handleCloseMoreOptions} />
    </AccountPageContainer>
  );
};
