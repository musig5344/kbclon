  ...
  ${({ $focused, $hasValue }) =>
    ...
    css`
      ...
      color: ${$focused
        ...
        : KBDesignSystem.colors.text.tertiary};
    `}
...
const InputWrapper = styled.div<{ $focused: boolean; $error: boolean }>`
  ...
  border: 1px solid
    ${({ $error, $focused }) =>
      $error
        ? KBDesignSystem.colors.status.error
        : $focused
          ? KBDesignSystem.colors.primary.yellow
...
const HelperText = styled.div<{ $error?: boolean }>`
  ...
  font-size: ${KBDesignSystem.typography.fontSize.xs};
  color: ${({ $error }) =>
    $error ? KBDesignSystem.colors.status.error : KBDesignSystem.colors.text.tertiary};
`;
    ...
    <InputContainer>
      <InputWrapper $focused={focused} $error={Boolean(error)}>
        ...
        {secure && variant === 'password' && (
          <ActionButton type='button' onClick={() => setShowPassword(!showPassword)}>
      ...
      {(error || helper) && <HelperText $error={Boolean(error)}>{error || helper}</HelperText>}
    </InputContainer>
  );
};
