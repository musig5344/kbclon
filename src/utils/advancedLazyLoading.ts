...
class AdvancedLazyLoadingManager {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(`✅ Chunk loaded: ${strategy.chunkName} in ${(metric.endTime - metric.startTime).toFixed(2)}ms (${attempt} retries)`);
          }
          ...
          if (attempt === maxRetries) {
            ...
            console.error(`❌ Chunk failed to load: ${strategy.chunkName} after ${attempt} retries`, lastError);
            throw lastError;
          }
   ...
   * 우선순위 기반 프리페치
   */
  async prefetchByPriority(routes: Array<{ priority: LoadingPriority; importFn: () => Promise<any>; chunkName: string }>) {
    // 우선순위별로 정렬
    const sortedRoutes = routes.sort((a, b) => {
      const priorities = [LoadingPriority.IMMEDIATE, LoadingPriority.HIGH, LoadingPriority.MEDIUM, LoadingPriority.LOW, LoadingPriority.ON_DEMAND];
...
export function createConditionalLazyRoute<T = React.ComponentType<any>>(
  ...
  return React.lazy(async () => {
    try {
      ...
      console.warn(`Conditional lazy loading failed, using fallback for ${strategy.chunkName}:`, error);
      return await fallbackFn();
    }
  });
}
