...
export function useStableActions<T extends Record<string, Function>>(actions: T): T {
  const stableActions = useMemo(() => {
    ...
    Object.keys(actions).forEach(key => {
      stableActionsObj[key as keyof T] = useCallback(actions[key as keyof T], [
...
export function useContextPerformance(contextName: string) {
  ...
  React.useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(
        `${contextName} Context 렌더링 #${renderCount.current}, 간격: ${timeSinceLastRender}ms`
      );
    }
...
>(initialState: State) {
  ...
  const Provider: React.FC<{
    ...
    children: ReactNode;
    state?: State;
  }> = ({ actions, children, state = initialState }) => {
    ...
    return (
      <StateContext.Provider value={stableState}>
        <ActionsContext.Provider value={stableActions}>{children}</ActionsContext.Provider>
 ...
 * Context 디버깅 도구
 */
export function createContextDebugger<T>(Context: React.Context<T>, contextName: string) {
