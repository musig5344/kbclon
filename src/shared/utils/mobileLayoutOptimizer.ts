...
import { WCAG_TOUCH_CONSTANTS, TouchDensity } from './touchTargetOptimizer';
...
export class MobileLayoutOptimizer {
  ...
  private detectUsageContext(): UsageContext[] {
    ...
    if (
      this.currentDevice === DeviceCategory.PHONE_SMALL ||
  ...
  calculateOptimalSize(baseSize?: number): number {
    ...
    const contextMultiplier = Math.max(
      ...this.currentContext.map(context => this.scalingConfig.contextAdjustments[context])
    );
  ...
  isInThumbFriendlyZone(
    element: HTMLElement,
    zone: 'easy' | 'comfortable' = 'comfortable'
  ): boolean {
  ...
  private updateLayout(): void {
    ...
    window.dispatchEvent(
      new CustomEvent('mobile-layout-updated', {
        detail: {
  ...
  auditMobileOptimization(): {
    ...
    const interactiveElements = document.querySelectorAll(
      ['button', 'input', 'select', 'textarea', 'a[href]', '[role="button"]', '[onclick]'].join(',')
    ) as NodeListOf<HTMLElement>;
...
export type { ResponsiveScalingConfig, ThumbReachZones };
