...
export const PullToRefresh: React.FC<PullToRefreshProps> = ({
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (diff > 0) {
        // 탄성 효과 적용
        const elasticPull = diff > threshold ? threshold + (diff - threshold) * 0.3 : diff;
  ...
  return (
    ...
    >
      <RefreshIndicator $isVisible={pullDistance > 20} $isRefreshing={isRefreshing}>
        <LogoIcon src={kbLogo} alt='KB' $isRefreshing={isRefreshing} $rotation={rotation} />
      </RefreshIndicator>

      <RefreshText $isVisible={pullDistance > 20}>{getRefreshText()}</RefreshText>

      <ScrollContainer ref={scrollContainerRef} $isPulling={isPulling} $pullDistance={pullDistance}>
        {children}
      </ScrollContainer>
    </Container>
  );
