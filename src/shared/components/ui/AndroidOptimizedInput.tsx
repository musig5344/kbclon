...
const StyledInput = styled.input<{ $hasError?: boolean; $variant?: 'outlined' | 'filled' }>`
  ...
  border: 1px solid ${({ $hasError }) => ($hasError ? '#FF3B30' : tokens.colors.border.primary)};
  border-radius: 8px;
  ...
  &:focus {
    border-color: ${({ $hasError }) => ($hasError ? '#FF3B30' : '#FFD338')};
    box-shadow: 0 0 0 2px
      ${({ $hasError }) => ($hasError ? 'rgba(255, 59, 48, 0.2)' : 'rgba(255, 211, 56, 0.2)')};
  }
...
const HelperText = styled.div<{ $isError?: boolean }>`
  font-size: 12px;
  color: ${({ $isError }) => ($isError ? '#FF3B30' : tokens.colors.text.tertiary)};
  margin-top: 4px;
...
export const AndroidEmailInput: React.FC<AndroidOptimizedInputProps> = props => (
  <AndroidOptimizedInput type='email' inputMode='email' autoComplete='email' {...props} />
);

export const AndroidPasswordInput: React.FC<AndroidOptimizedInputProps> = props => (
  <AndroidOptimizedInput type='password' autoComplete='current-password' {...props} />
);
