...
const frames = Array.from(
  { length: 17 },
  (_, i) => `/src/assets/images/loading/loading_1_${String(i + 1).padStart(2, '0')}.png`
);
...
export const UnifiedLoading: React.FC<UnifiedLoadingProps> = memo(
  ({
    ...
    return (
      <LoadingWrapper $type={type} $isVisible={isVisible} $blur={blur} className={className}>
        <LoadingContent $size={size}>
          <AnimationImage src={frames[currentFrameIndex]} alt='KB 스타뱅킹 로딩' $size={size} />
          {message && (
...
export const LoadingScreen: React.FC<{ text?: string }> = ({ text }) => (
  <UnifiedLoading type='fullscreen' message={text} size='medium' />
);

export const InlineLoading: React.FC<{ text?: string; size?: 'small' | 'medium' | 'large' }> = ({
  text,
  size = 'small',
}) => <UnifiedLoading type='inline' message={text} size={size} />;

export const OverlayLoading: React.FC<{ text?: string; blur?: boolean }> = ({
  text,
  blur = true,
}) => <UnifiedLoading type='overlay' message={text} size='medium' blur={blur} />;
