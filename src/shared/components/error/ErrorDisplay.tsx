...
const ErrorContainer = styled.div<{ variant: 'inline' | 'card' | 'banner' }>`
  ...
  padding: ${props => {
    switch (props.variant) {
      case 'inline': return '8px 0';
      case 'card': return '16px';
      case 'banner': return '12px 16px';
      default: return '12px';
    }
  }};
  background-color: ${props => {
    switch (props.variant) {
      case 'inline': return 'transparent';
      case 'card': return '#fff5f5';
      case 'banner': return '#fef2f2';
...
const getDefaultMessage = (type: ErrorType): string => {
  switch (type) {
    ...
    case 'validation': return '입력한 정보를 확인해 주세요.';
    case 'server': return '서버에 일시적인 문제가 발생했습니다.';
