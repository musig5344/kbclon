...
const SnackbarAction = styled.button<{
  ...
  color: ${props =>
    props.$variant === 'filled' ? tokens.colors.white : tokens.colors.brand.primary};

  &:hover {
    background-color: ${props =>
      props.$variant === 'filled' ? 'rgba(255, 255, 255, 0.08)' : 'rgba(255, 211, 56, 0.08)'};
  }

  &:active {
    background-color: ${props =>
      props.$variant === 'filled' ? 'rgba(255, 255, 255, 0.12)' : 'rgba(255, 211, 56, 0.12)'};
  }
`;
  ...
  return (
    <SnackbarContainer $open={isVisible} $position={position}>
      <SnackbarContent $severity={severity} $variant={variant}>
        {severity !== 'default' && <SnackbarIcon>{getIcon(severity)}</SnackbarIcon>}

        <SnackbarMessage>{message}</SnackbarMessage>

        {action && (
          <SnackbarAction onClick={handleActionClick} $variant={variant}>
            {action.label}
          </SnackbarAction>
        )}
      </SnackbarContent>
