...
export type ToastPosition = 'top' | 'top-right' | 'top-left' | 'bottom' | 'bottom-right' | 'bottom-left' | 'center';
...
const ToastItem = styled.div<{ 
  ...
  ${props => {
    if (props.$isExiting) {
      return css`animation: ${fadeOut} ${duration.fast} ${easing.easeIn} forwards;`;
    }
    ...
    switch (props.$position) {
      ...
      case 'top-left':
      case 'top-right':
        return css`animation: ${props.$position === 'top' ? slideInTop : props.$position === 'top-right' ? slideInRight : slideInLeft} ${duration.normal} ${easing.easeOut};`;
      case 'bottom':
      case 'bottom-left':
      case 'bottom-right':
        return css`animation: ${props.$position === 'bottom' ? slideInBottom : props.$position === 'bottom-right' ? slideInRight : slideInLeft} ${duration.normal} ${easing.easeOut};`;
      case 'center':
        return css`animation: ${fadeIn} ${duration.normal} ${easing.easeOut};`;
    }
  }}
`;
...
const Toast: React.FC<ToastOptions & {
  ...
  return (
    <ToastItem $type={type} $position={position} $isExiting={isExiting}>
      ...
      {showProgress && duration > 0 && !isExiting && (
