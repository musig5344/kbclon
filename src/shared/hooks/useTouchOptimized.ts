...
export function useTouchOptimized(
  ...
  const updateGesture = useCallback((touches: Touch[] | TouchList) => {
    ...
    if (normalizedTouches.length >= 2 && initialTouches.current && initialTouches.current.length >= 2) {
  ...
  // Detect gesture type
  const detectGestureType = useCallback((state: GestureState): GestureType | null => {
    // Force touch
    ...
    if (state.distance > touchConfig.tapMovementThreshold) {
      if (state.velocityX > touchConfig.swipeVelocityThreshold ||
