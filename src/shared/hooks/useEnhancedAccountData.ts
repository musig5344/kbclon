...
export const useEnhancedAccountData = (): UseEnhancedAccountDataResult => {
  ...
  const fetchAccounts = useCallback(async (isRetry = false, abortController?: AbortController) => {
      ...
      if (isMountedRef.current && !controller.signal.aborted) {
        if (response.success && response.data) {
          ...
          if (isAndroid && response.networkStatus) {
            ...
            safeLog('info', `Android WebView - 네트워크: ${isOnline ? '온라인' : '오프라인'} (${connectionType}, RTT: ${rtt}ms)`);
          }
